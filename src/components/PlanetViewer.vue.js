...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        // 相機動畫追蹤
        let globalCameraAnimation = null;
        // 滾輪控制狀態
            ...
            planets.forEach((planetData, index) => {
                ...
                // 特殊處理地球，添加雲層
                    ...
                    if (planetData.name === "火星") {
                        material.bumpMap = loadTexture('/assets/planets/marsbump.jpg');
                        material.bumpScale = 0.35; // 增強火星表面紋理
                    }
                    else if (planetData.name === "金星") {
                        material.bumpMap = loadTexture('/assets/planets/venusbump.jpg');
                        material.bumpScale = 0.25;
                    }
                    else if (planetData.name === "水星") {
                        material.bumpMap = loadTexture('/assets/planets/mercurybump.jpg');
                        material.bumpScale = 0.22;
                    }
                    else if (planetData.name === "月球") {
                        material.bumpMap = loadTexture('/assets/planets/moonbump.jpg');
        ...
        const addSciFiFrame = (planetMesh, planetSize) => {
            // 創建一個組來容納所有框線
            ...
            cornerPositions.forEach((corner, index) => {
                // 計算每條線的終點
                const xEnd = new THREE.Vector3(corner.x > 0 ? corner.x - cornerLength : corner.x + cornerLength, corner.y, corner.z);
                const yEnd = new THREE.Vector3(corner.x, corner.y > 0 ? corner.y - cornerLength : corner.y + cornerLength, corner.z);
                const zEnd = new THREE.Vector3(corner.x, corner.y, corner.z > 0 ? corner.z - cornerLength : corner.z + cornerLength);
                // X方向線
        ...
        const handleWheel = (event) => {
            ...
            if (currentFocusIndex < 0)
                currentFocusIndex = planetObjects.length - 1;
            if (currentFocusIndex >= planetObjects.length)
                currentFocusIndex = 0;
            // 獲取當前焦點行星
